...
Importer.prototype.getQueue = function () {
  ...
  if (!this.queue) {
    this.queue = async.queue(function (candle, cb) {
      self.getModel().make(candle).save(function (err, doc) {
        if (err) {
          console.error(err);
        }
        if (err) {
  ...
  if (!this.hasStream(stream)) {
    var c = csv()
      ...
      .on('record', function (record, index) {
        ...
        self.getHarvester().harvest(moment.unix(Number(record[0])).valueOf(), Number(record[1]), Number(record[2]));
      })
      .on('end', function () {
